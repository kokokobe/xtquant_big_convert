...
def _load_local_config():
    # Precedence (2026-09-15): the operator's external
    # bigqmt_signal_trader_local_config.py (deployed beside the strategy in
    # QMT's python dir) wins over the embedded placeholder block. The
    # placeholder carries YOUR_ACCOUNT_ID and transport=zmq, so shadowing the
    # external file silently broke per-account wiring -- queue names, shm
    # section names and derived ports all drifted to placeholder values
    # (bridge bound 15560 and ran as account "YOUR_ACCOUNT_ID" while the
    # operator's config said 52625295 + shm). A stale module left by a
    # previous strategy load must not survive either: whenever the external
    # file exists we re-exec it from disk; only when it does not exist do we
    # fall back to the embedded placeholder (marked __bigqmt_flat_builtin__).
    external_path = None
    for entry in sys.path:
        if not entry or not os.path.isdir(entry):
            continue
        candidate = os.path.join(entry, "bigqmt_signal_trader_local_config.py")
        if os.path.isfile(candidate):
            external_path = candidate
            if entry.endswith(os.sep + "python"):
                break
    if external_path is not None:
        import importlib.util as _importlib_util
        spec = _importlib_util.spec_from_file_location(
            "bigqmt_signal_trader_local_config", external_path)
        module = _importlib_util.module_from_spec(spec)
        sys.modules["bigqmt_signal_trader_local_config"] = module
        spec.loader.exec_module(module)
        print("[bigqmt_shell] external local config used: %s" % external_path)
        return module
...
try:
    ...
    # Default is zmq (the historical no-redis wire). An explicit "shm" in the
    # local config opts into the zero-TCP transport (2026-09-15) and is
    # honored -- anything else keeps the zmq force so a typo cannot fall
    # through to a redis attempt that would trip the terminal audit scan.
    BIGQMT_REDIS_CONFIG = dict(BIGQMT_REDIS_CONFIG or {})
    if str(BIGQMT_REDIS_CONFIG.get("transport") or "").lower() != "shm":
    ...
    print("[bigqmt_shell] no-redis mode: transport=%s background_threads=%s "
          "redis_enabled=False" % (BIGQMT_REDIS_CONFIG["transport"],
                                   BIGQMT_REDIS_CONFIG["rpc_background_threads"]))
